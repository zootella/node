...
function Task(next) {//monitor the completion of an asynchronous call
	...
	o.pulse = function() {//and the program will pulse it for us
		if (startTime.expired(Time.out)) {
			close(o);
			if (next) next(Result(startTime, Mistake("timeout"), null));
		}
	}

	o.fail = function (error) {//e error
		if (o.isClosed()) return;
		close(o);
		if (next) next(Result(startTime, error, null));
	}

	o.done = function (answer) {//a answer
		if (o.isClosed()) return;
		close(o);
		if (next) next(Result(startTime, null, answer));
	}

	return o;
};

//a Result tells how an asynchronous task ended
...
function Result(startTime, error, answer) {
	var o = {};
